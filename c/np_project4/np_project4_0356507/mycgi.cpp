...
int main(void) {
	...
	string socks_server_name[MAX_CLIENT];
	int socks_port[MAX_CLIENT];
	...
	while (count < 5 * MAX_CLIENT) {
		eq = qstr.find_first_of("=", ampersand + 1);
		ampersand = qstr.find_first_of("&", eq + 1);
		if (count % 5 == 0) {
			hosts[count / 5] = qstr.substr(eq + 1, ampersand - eq - 1);
		}
		else if (count % 5 == 1 ) {
			ports[count / 5] = atoi(qstr.substr(eq + 1, ampersand - eq - 1).c_str());
		}
		else if (count % 5 == 2 ) {
			data_str[count / 5] = qstr.substr(eq + 1, ampersand - eq - 1);
		}
		else if (count % 5 == 3 ) {
			socks_server_name[count / 5] = qstr.substr(eq + 1, ampersand - eq - 1);;
		}
		else if (count % 5 == 4 ) {
			if (count == 5 * MAX_CLIENT - 1)
				socks_port[count / 5] = atoi(qstr.substr(eq + 1).c_str());
			else
				socks_port[count / 5] = atoi(qstr.substr(eq + 1, ampersand - eq - 1).c_str());
		}
	...
	char ip_dotted_name[MAX_CLIENT][100];
	int first_time_read[MAX_CLIENT];
	...
	for (int i = 0; i < MAX_CLIENT; ++i)
	{
		...
		if (!hosts[i].empty()) {
			if ((he = gethostbyname(socks_server_name[i].c_str())) == NULL) {
			...
			client_sin.sin_port = htons(socks_port[i]);
		...
		for (int i = 0; i < MAX_CLIENT; ++i)
		{
			if (!hosts[i].empty()) {
				...
				if (status[i] == F_CONNECTING && (FD_ISSET(sock_fd[i], &rfds) || FD_ISSET(sock_fd[i], &wfds)) ) {
					if ( getsockopt(sock_fd[i], SOL_SOCKET, SO_ERROR, &err, (socklen_t *)&n) < 0 && errno != EINPROGRESS ) {
						perror("getsockopt");
						printf("<script>document.all['m%d'].innerHTML += \"sock%d connect fail: %s<br>\";</script>\n", i, i, strerror(errno));
						fflush(stdout);
						exit(1);
					...
					req[0] = 4;
					req[1] = 1; // CONNECT mode
					req[2] = ports[i] / 256;
					req[3] = ports[i] % 256;

					if ((he2 = gethostbyname(hosts[i].c_str())) == NULL) {
						perror("gethostbyname");
						exit(1);
					}
					snprintf(ip_dotted_name[i], sizeof(ip_dotted_name[i]), "%s", inet_ntoa(*((struct in_addr *)he2->h_addr)) );
					// cout << ip_dotted_name[i] << endl;

					char *u = strtok(ip_dotted_name[i], ".");
					req[4] = atoi(u);
					...
					if (first_time_read[i] == 1) {
						first_time_read[i] = 0;
						read(sock_fd[i], buf, 8);
					}
					else {
						memset(buf, 0, 65536);
